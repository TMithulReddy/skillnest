...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('<div class="dashboard-container">')) {
        dStartIdx = i;
    }
...
const templateTop = lines.slice(0, dStartIdx + 1).join('\\n');
const templateBottom = lines.slice(dEndIdx - 1).join('\\n');

function generatePage(title, activeLink, headerTitle, headerSub, innerContent, fileName) {
    let html = templateTop + "\\n" + innerContent + "\\n" + templateBottom;
    
    html = html.replace('<title>Dashboard - SkillNest</title>', \`<title>\${title} - SkillNest</title>\`);
    html = html.replace(/class="sidebar-link active"/gi, 'class="sidebar-link"');
    
    // Very simple replace for active string link by assuming explicit href
    html = html.replace(\`href="\${activeLink}" class="sidebar-link"\`, \`href="\${activeLink}" class="sidebar-link active"\`);
...
generatePage('My Sessions', 'sessions.html', 'My Sessions', 'Organize your upcoming and completed meetings', sessionsHtml, 'sessions.html');
generatePage('Chat', 'chat.html', 'Messages', 'Real-time peer communication', chatHtml, 'chat.html');
generatePage('Credits', 'credits.html', 'Credit Balance', 'Track your ecosystem economy', creditsHtml, 'credits.html');

console.log("Basic explicit build script output OK.");
